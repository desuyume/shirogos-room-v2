...
const RoomGuidePreviewRoomList: FC = () => {
	...
	return isLoading ? (
		...
		<div
			ref={ref}
			className={
				'flex-1 h-full flex flex-col justify-between room-guide-preview-list ' +
				(inView ? 'inView' : '')
			}
		>
			<div className='w-full h-[48%] bg-secondary bg-opacity-90 rounded-tl-[2.3125rem] rounded-bl-[2.3125rem]'>
				<div className='w-full h-full bg-tertiary bg-opacity-40 rounded-tl-[2.3125rem] rounded-bl-[2.3125rem] px-8 flex items-center'>
