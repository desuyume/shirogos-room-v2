...
const EditorMainInfo: FC<IEditorMainInfo> = ({
  ...
  return (
    <div className='mb-3 box-content flex h-[45.4375rem] w-full flex-col items-center border-[5px] border-primary bg-secondary'>
      <EditorMainInfoInput state={id} setState={setId} type='id' title='ID персонажа' />
      <EditorMainInfoInput state={name} setState={setName} type='name' title='Имя персонажа' />
      <EditorMainInfoInput
        state={subtitle}
      ...
      <EditorMainInfoAvatar avatar={avatar} setAvatar={setAvatar} setMiniature={setMiniature} />
    </div>
  )
}
