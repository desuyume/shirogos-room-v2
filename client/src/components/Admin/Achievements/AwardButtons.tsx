  ...
  return (
    <div className='flex'>
      <button
        ...
        onClick={() =>
          selectedAwardType === 'badge' ? setSelectedAwardType(null) : setSelectedAwardType('badge')
        }
        className={
          (awardType.includes('badge') ? 'hover:bg-primary ' : '') +
          (selectedAwardType === 'badge' ? 'bg-primary ' : '') +
          'h-4 w-1/4 border-[1px] border-primary transition-all disabled:border-secondary'
        }
      />
      <button
        disabled={!awardType.includes('frame')}
        onClick={() =>
          selectedAwardType === 'frame' ? setSelectedAwardType(null) : setSelectedAwardType('frame')
        }
        className={
