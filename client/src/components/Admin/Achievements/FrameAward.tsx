...
interface IFrameAward {
    selectedAwardType: string | null
    setFrameImg: React.Dispatch<React.SetStateAction<File | null>>
}

const FrameAward: FC<IFrameAward> = ({ selectedAwardType, setFrameImg }) => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            setFrameImg(e.target.files[0])
        }
    }

    return (
        <div className={(selectedAwardType === 'frame' ? 'block ' : 'hidden ') + 'w-full flex-1 flex justify-center items-center transition-all relative'}>
            <input
                ref={inputRef}
