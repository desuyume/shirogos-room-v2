...
const RoleSwitcher: FC<IRoleSwitcher> = ({ type }) => {
  ...
  useEffect(() => {
    if (isFetchedAfterMount && isSuccess) {
      if (type === 'adjective' && activeRole !== uniqueRoles.selected_unique_role_adjective) {
        mutate({ role: activeRole })
      }

      if (type === 'noun' && activeRole !== uniqueRoles.selected_unique_role_noun) {
        mutate({ role: activeRole })
      }
    }
  }, [activeRole])
  ...
  return (
    <div className='relative flex h-[1.6875rem] w-full items-center'>
      {isLoading ? (
        <p className='h-full w-full text-center text-primaryText'>загрузка...</p>
      ) : isError ? (
        <p className='h-full w-full text-center text-primaryText'>ошибка 0_0</p>
      ) : (
        <>
          <button
            ...
            className={
              ...
              'absolute left-0 h-full w-[2%] transition-all hover:w-[5%] disabled:opacity-50 disabled:hover:w-[2%]'
            }
          />
          <p className='flex-1 text-center text-[0.9375rem] text-primaryText'>
          ...
          <button
            ...
            className={
              (type === 'adjective'
                ? 'bg-primaryText '
                : `${colorVariants.bg[roomAppearance.active_room_color]} `) +
              'absolute right-0 h-full w-[2%] transition-all hover:w-[5%] disabled:opacity-50 disabled:hover:w-[2%]'
            }
          />
        </>
      )}
