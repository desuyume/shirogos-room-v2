...
const EditorElement: FC<IEditorElement> = ({
	...
	const recalculateBadgePercentSize = (width: number, height: number) => {
		const container = document.querySelector('.editor-container') as HTMLElement
		if (!elementRef.current) return
		const badgeSizeInPercent = getEditorBadgePercentSize(
			container,
			width,
			height
		)
	...
	return (
		...
		>
			<div
				...
				className={`inline-block absolute ` + (!!className ? className : '')}
				style={{ zIndex: zIndex }}
			>
				{type === 'widget' ? (
					element
					...
					<Resizable
						...
						onResizeStop={(e, __, ___, d) => {
							...
							recalculateBadgePercentSize(
								badgeSize.width + d.width,
								badgeSize.height + d.height
							)
