...
const PrimaryHeaderBttn: FC<IPrimaryHeaderBttn> = ({
	path,
	title,
	room_color,
}) => {
			...
			className={
				(isActive
					...
					? `${colorVariants.bg[roomAppearance.active_room_color]} ${
					  ...
					  } text-primaryText hover:text-white cursor-pointer `
					: pathname.includes('guide') && room_color
					? `${colorVariants.bg[room_color]} ${colorVariantsHover.bg[room_color]} text-primaryText hover:text-white cursor-pointer `
					: `bg-primary hover:bg-primaryHover text-primaryText hover:text-white cursor-pointer `) +
